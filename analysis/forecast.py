...
def prepare_data(df, end_date=None, fill_missing_with_zero=False):
    ...
    if end_date is not None:
        date_index = pd.date_range(prophet_df.index.min(), pd.to_datetime(end_date), freq='D')
        date_index.name = 'ds'
        prophet_df = prophet_df.reindex(date_index)
    else:
        prophet_df = prophet_df.asfreq('D')

    if fill_missing_with_zero:
        prophet_df = prophet_df.fillna({'y': 0})
    else:
        prophet_df = prophet_df.interpolate()
...
def forecast_next_days(model, days_ahead):
    ...
    # Posting counts cannot be negative or fractional. Normalize every forecast
    # value before it is returned to the API and plotted in the chart.
    forecast_output = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(days_ahead).copy()
    numeric_columns = ['yhat', 'yhat_lower', 'yhat_upper']
    forecast_output[numeric_columns] = (
        forecast_output[numeric_columns].clip(lower=0).round().astype(int)
    )
