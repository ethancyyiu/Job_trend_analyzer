    ...
    <main className="overview-page">
      ...
      <section className="overview-bottom-grid">
        <div><SectionHeading title="Most in-demand skills" caption="By posting count · tracked listings" />
          <div className="overview-skills-scroll"><div className="overview-skills">{skills === undefined ? <div className="overview-state">Loading skills…</div> : topSkills.length ? topSkills.map((skill) => <div className="overview-skill" key={skill.skill}><div><span>{String(skill.skill).toUpperCase()}</span><small>{formatNumber(skill.count)}</small></div><i><b style={{ width: `${(Number(skill.count) / highestSkillCount) * 100}%` }} /></i></div>) : <div className="overview-state">No skill data is available yet.</div>}</div></div>
        </div>
