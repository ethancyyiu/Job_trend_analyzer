...
export function Salary({ cachedData }) {
  ...
  return <main className="salary-page">
    ...
    <section className="salary-insights" aria-label="Salary insights">
      <article className="salary-insight-card"><Heading title="Listed salary bands" caption="Annualized role midpoints with disclosed pay" />
        {cachedData === undefined ? <div className="salary-insight-state">Loading salary distribution...</div> : salaryBands.length ? <div className="salary-band-list">{salaryBands.map((band) => <div className="salary-band" key={band.label}><span>{band.label}</span><i><b style={{ width: `${(Number(band.count) / largestBand) * 100}%` }} /></i><strong>{Number(band.count).toLocaleString()}</strong></div>)}</div> : <div className="salary-insight-state">No annual salary data is available yet.</div>}
      </article>
      <article className="salary-insight-card"><Heading title="Pay type mix" caption="Disclosed compensation records" />
        {cachedData === undefined ? <div className="salary-insight-state">Loading pay types...</div> : payTypeTotal ? <div className="salary-type-content"><div className="salary-type-bar"><i style={{ width: `${salaryMix}%` }} /></div><div className="salary-type-labels"><span><b>{salaryMix.toFixed(0)}%</b> Yearly</span><span><b>{(100 - salaryMix).toFixed(0)}%</b> Hourly</span></div><small>{payTypeTotal.toLocaleString()} records include a pay type</small></div> : <div className="salary-insight-state">No pay-type data is available yet.</div>}
      </article>
